...
IMPORTANT: You could also use frameworks like LangChain or CrewAI instead of or in conjunction with Llama Stack in OpenShift AI. All of these tools help you build agentic AI workflows with reasoning, tool use, and orchestration. Llama Stack is Red Hat's recommended, and supported, framework for standardizing and interweaving disparate pieces of an agentic system. 

* **Kubernetes MCP Server**  - allows our LLM agent to interact with our live OpenShift cluster. This MCP server is namespace-scoped. We will use it to interact with the OpenShift resources within the `lls-demo` namespace.
...
You can connect a Llama Stack server deployment to any model endpoint you desire. For our workshop, our Llama Stack server instance is configured with our Qwen model and two MCP servers. 
...
You are now setup to use the chat interfaces to test the model and its functionality with our MCP servers.
...
The tool information you just viewed will give you guidance into how to interact with the model in chat to activate the tool calls correctly. You will need to utilize keywords so that you're engaging with the tools you're aiming for.
