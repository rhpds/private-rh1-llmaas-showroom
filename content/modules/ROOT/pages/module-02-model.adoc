...
Now, go to the GenAI Studio section of the dashboard and select `AI asset endpoints`.
...
We do, however, have a model deployed in a separate namespace managed by our cluster admin. This admin has set up a `Model as a Service` architecture for our organization, with a central model deployed and managed, available for use by disparate teams. 
...
Now, click `Generate an API token`.
