...
We do, however, have a model deployed in a separate namespace managed by our cluster admin. This admin has set up a `Model as a Service` archiecture for our organization, with a central model deployed and managed, available for use by disparate teams. 
